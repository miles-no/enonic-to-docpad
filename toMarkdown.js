...
var toMarkdown = function(string) {
  ...
  var ELEMENTS = [
    ...
    },
    {
      patterns: 'a',
      replacement: function(str, attrs, innerHTML) {
        var href = attrs.match(attrRegExp('href'))[1],
            title = attrs.match(attrRegExp('title'));

        if(href.indexOf('_attachment') >= 0){
          href = '/media/' + href.match(/\/.*\/_attachment\/(\d*)/)[1] + '.pdf';
        }
        return href ? '[' + innerHTML + ']' + '(' + href.toLowerCase() + (title && title[1] ? ' "' + title[1] + '"' : '') + ')' : str;
      }
    },
    {
      patterns: ['b', 'strong'],
